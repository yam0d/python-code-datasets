...
#Extracting tweets from the official handle of BJP

Bjp_tweets2 = searchTwitter("@BJP4India",since="2019-05-15",until="2019-05-21", n=5000,lang="en")
...
p <- plot_ly(emo_sum, x=~emotion, y=~count, type="bar", color=~emotion) %>%
  layout(xaxis=list(title=""), showlegend=FALSE,
         title="Emotion for BJP")

p

# Create comparison word cloud data
wordcloud_tweet = c(
  paste(Bjp_tweets2$text[emotions$anger > 0], collapse=" "),
  paste(Bjp_tweets2$text[emotions$anticipation > 0], collapse=" "),
  paste(Bjp_tweets2$text[emotions$disgust > 0], collapse=" "),
  paste(Bjp_tweets2$text[emotions$fear > 0], collapse=" "),
  paste(Bjp_tweets2$text[emotions$joy > 0], collapse=" "),
  paste(Bjp_tweets2$text[emotions$sadness > 0], collapse=" "),
  paste(Bjp_tweets2$text[emotions$surprise > 0], collapse=" "),
  paste(Bjp_tweets2$text[emotions$trust > 0], collapse=" ")
)
